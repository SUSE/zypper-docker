test_stable ::
	...
	docker run --rm -v `pwd`:/go/src/github.com/SUSE/zypper-docker zypper-docker-testing-golang-stable climate -open=false -threshold=80.0 -errcheck -vet -fmt -race .
...
test_tip ::
	...
	docker run --rm -v `pwd`:/go/src/github.com/SUSE/zypper-docker zypper-docker-testing-golang-tip climate -open=false -threshold=80.0 -errcheck -vet -fmt -race .
